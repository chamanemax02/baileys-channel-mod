...
export const extractImageThumb = async (bufferOrFilePath, width = 32) => {
    try {
        ...
        else if ('jimp' in lib && (typeof lib.jimp?.Jimp === 'object' || typeof lib.jimp?.Jimp === 'function' || typeof lib.jimp?.read === 'function' || typeof lib.jimp?.default?.read === 'function')) {
            const JimpClass = lib.jimp.Jimp || lib.jimp.default?.Jimp || lib.jimp.default || lib.jimp;
            const readFn = JimpClass.read || lib.jimp.read || lib.jimp.default?.read;
            const jimp = await readFn.call(JimpClass, bufferOrFilePath);
            const dimensions = {
                width: jimp.width || jimp.bitmap?.width || 100,
                height: jimp.height || jimp.bitmap?.height || 100
            };
            let buffer;
            if (typeof jimp.getBuffer === 'function') {
                buffer = await jimp.getBuffer('image/jpeg', { quality: 50 });
            } else if (typeof jimp.getBufferAsync === 'function') {
                buffer = await jimp.getBufferAsync('image/jpeg');
            } else {
                buffer = await jimp.getBuffer('image/jpeg');
            }
            return {
                buffer,
                original: dimensions
        ...
        // Return dummy empty buffer thumbnail to prevent message generation failure
    }
    return {
