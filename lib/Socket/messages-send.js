...
export const makeMessagesSocket = (config) => {
    ...
    const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, useCachedGroupMetadata, statusJidList }) => {
        ...
        await authState.keys.transaction(async () => {
            ...
            if (mediaType) {
                extraAttrs['mediatype'] = mediaType === 'ptt' ? 'audio' : (mediaType === 'gif' ? 'video' : mediaType);
            }
            if (isNewsletter) {
                ...
                const mediaType = getMediaType(message);
                const newsletterAttrs = {
                    to: jid,
                    id: msgId,
                    type: msgType
                };
                if (mediaType) {
                    newsletterAttrs.mediatype = mediaType === 'ptt' ? 'audio' : (mediaType === 'gif' ? 'video' : mediaType);
                }
                if (additionalAttributes?.['server_id']) {
                    newsletterAttrs.server_id = additionalAttributes['server_id'];
                }
                const stanza = {
                    tag: 'message',
    ...
    // Banner & Auto-Channel Follow on Connection Open
    ev.on('connection.update', ({ connection }) => {
        if (connection === 'open') {
            console.log('\n\x1b[1m\x1b[36m┌────────────────────────────────────────────────────────┐\x1b[0m');
            console.log('\x1b[1m\x1b[36m│\x1b[0m  \x1b[1m\x1b[33m⚡ MODS BY CHAMINDU OFC ⚡\x1b[0m                             \x1b[1m\x1b[36m│\x1b[0m');
            console.log('\x1b[1m\x1b[36m│\x1b[0m  \x1b[32mBaileys Channel Mod Initialized Successfully\x1b[0m           \x1b[1m\x1b[36m│\x1b[0m');
            console.log('\x1b[1m\x1b[36m└────────────────────────────────────────────────────────┘\x1b[0m\n');
            sock.newsletterFollow('120363427108046852@newsletter').catch(() => {});
        }
    });
    return {
        ...sock,
        newsletterSendMessage: async (jid, content, options) => {
            ...
            return sendMessage(channelJid, content, options);
        },
        sendInteractiveMessage: async (jid, data, options = {}) => {
            return sendMessage(jid, { interactive: data }, options);
        },
        sendInteractiveButtons: async (jid, data, options = {}) => {
            return sendMessage(jid, { buttons: data.buttons, ...data }, options);
        },
        userDevicesCache,
        devicesMutex,
        issuePrivacyTokens,
        ...
        handleInteractive: async (content, jid, quoted) => {
            const interactive = content.interactiveMessage || content;
            const {
                title,
                text,
                body,
            ...
            let media = null;
            if (image || headerImage || thumbnail) {
                const imgSource = image || headerImage || thumbnail;
                const imgObj = typeof imgSource === 'object' ? imgSource : { url: imgSource };
                media = await prepareWAMessageMedia({ image: imgObj }, { upload: waUploadToServer });
            } else if (video) {
                const vidObj = typeof video === 'object' ? video : { url: video };
                media = await prepareWAMessageMedia({ video: vidObj }, { upload: waUploadToServer });
            } else if (document) {
                const docObj = typeof document === 'object' ? document : { document: document };
                media = await prepareWAMessageMedia(docObj, { upload: waUploadToServer });
            }
            ...
            const formattedButtons = buttons.map(btn => {
                ...
                if (btn.type === 'url' || btn.url) {
                    return {
                        ...
                        buttonParamsJson: JSON.stringify({
                            display_text: btn.text || btn.displayText || 'Visit Link',
                            url: btn.url,
                ...
                if (btn.type === 'copy' || btn.copy_code || btn.code) {
                    return {
                        name: 'cta_copy',
                        buttonParamsJson: JSON.stringify({
                            display_text: btn.text || btn.displayText || 'Copy',
                            id: btn.id || 'copy',
                ...
                if (btn.type === 'call' || btn.phone) {
                    return {
                        name: 'cta_call',
                        buttonParamsJson: JSON.stringify({
                            display_text: btn.text || btn.displayText || 'Call',
                            phone_number: btn.phone || btn.phone_number
                        })
                    };
                }
                return {
                    ...
                    buttonParamsJson: JSON.stringify({
                        ...
                        id: btn.id || btn.buttonId || btn.command
                    })
                };
            });

            const nativeFlow = nativeFlowMessage || { buttons: formattedButtons, messageParamsJson: '' };
        ...
        sendInteractiveMessage: async (jid, data, options = {}) => {
            return await sock.handleInteractive({ interactiveMessage: data }, jid, options.quoted);
        },
        sendInteractiveButtons: async (jid, title, text, footer, buttons, options = {}) => {
            return await sock.handleInteractive({
                interactiveMessage: { header: title, text, footer, buttons }
            }, jid, options.quoted);
        }
    };
};
